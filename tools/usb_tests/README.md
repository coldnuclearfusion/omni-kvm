...
| `usb_stress.py PORT SECONDS PREVENT STATS_PER_S` | Loads the serial link both ways (experiment A5). |
| `line_state_variants.py PORT` | Windows: which orders of raising DTR and RTS make the board count a daemon as connected (docs/platform.md, section 3). |
| `mac_hupcl_test.py PORT` | macOS: whether the board notices a process closing and reopening its port. |
...
| `input_driver.swift`, `run-input-driver.command` | Mac: posts HID-level keyboard and pointer events on request (`hotkey` = Command+Esc, `shift-down`, `shift-up`, `push-left N`, `push-right N`, `where`), read from `/tmp/mac_input.cmd`. Never clicks or types characters. Build with `swiftc -O input_driver.swift -o input_driver` next to `run-input-driver.command`, then start it with `open run-input-driver.command` (Terminal's permission lets it post). The Windows daemon ignores injected input by design, so there is no PC counterpart. |

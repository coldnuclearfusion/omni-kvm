...
- **Key**: 256-bit. Until pairing exists (Phase 5), both boards are built with the same development key from `firmware/include/secrets/dev_key.h`, generated by `tools/make_dev_key.py` and never committed.
...
- **Still to verify**: the 24 Mbps link at realistic desk distances (1–3 m, obstacles).
